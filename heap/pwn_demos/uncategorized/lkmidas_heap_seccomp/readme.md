# lkmidas's glibc 2.31 heap + seccomp exploitation technique using ROP
...
- My ROP chain is an `open-read-write` chain, which means that the only `syscalls` that need to be allowed by `seccomp` are `open`, `read` and `write` (this is true for most challenges). It doesn’t need `mmap` or `mprotect` to work. - You can write to a heap chunk that is large enough to contain the payload, which is 0x168 bytes, if it’s too much, you can consider using other shorter payloads with `mprotect` or `mmap`, but the idea is the same (and of course you must be able to free that chunk).
...
<details>
<summary><strong>Gadgets</strong></summary>
<p>
...
All the gadgets except `syscall_ret` can be found using this ROPgadget tool. For `syscall_ret`, you can use this command (thanks @Catafact for this):

```bash
objdump -D -Mintel ./libc-2.31.so | grep -B 1 ret | grep -A 1 syscall
```

The next 2 gadgets are crucial for this technique, the first one is at:
...
The next gadget is inside libc function `setcontext()`, at offset:
...
<details>
<summary><strong>The payload</strong></summary>
<p>
...
Nothing much to say about this part, if you know ROP, you know that this is an ordinary `open-read-write`
